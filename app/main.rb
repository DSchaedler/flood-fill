...
def tick_zero(args)
  ...
  $pixels_holding ||= []
  $pixels_holding << { x: $center[:x], y: $center[:y] }
...
def flood_fill(args)
  ...
  return if $pixels_to_check.empty?
  ...
  while times > 0
    ...
    $pixels_holding << next_pixel0 unless $pixels[next_pixel0[:x]][next_pixel0[:y]]

    next_pixel1 = { x: pixel[:x] + PIXEL_SIZE, y: pixel[:y] }
    $pixels[next_pixel1[:x]] ||= {}
    $pixels_holding << next_pixel1 unless $pixels[next_pixel1[:x]][next_pixel1[:y]]

    next_pixel2 = { x: pixel[:x], y: pixel[:y] - PIXEL_SIZE }
    $pixels[next_pixel2[:x]] ||= {}
    $pixels_holding << next_pixel2 unless $pixels[next_pixel2[:x]][next_pixel2[:y]]

    next_pixel3 = { x: pixel[:x], y: pixel[:y] + PIXEL_SIZE }
    $pixels[next_pixel3[:x]] ||= {}
    $pixels_holding << next_pixel3 unless $pixels[next_pixel3[:x]][next_pixel3[:y]]
